...
- Notifications: Discord webhook (`core/notifications/discord.py`), called from Celery tasks; no-ops when `DISCORD_WEBHOOK_URL` is empty.
...
- DRF surface (consumed by the SPA) is at `core/api/v1/urls.py` — auth (Google + token), profile/onboarding, preferences, assessments, plans, subscriptions (incl. `stream/` SSE, `checkout/`, `upgrade-quote/`, `cancel-pending/`, `<pk>/recheck/`), Mayar webhook, dashboard stats.
...
- `SECRET_KEY`, `DEBUG`, `ALLOWED_HOSTS`, DB, Celery, OpenAI, Mayar, Google OAuth, Discord, Apify, and CORS/CSRF origins are all read from env (see `.env.example`). `SECRET_KEY` falls back to a hardcoded dev value — never rely on that fallback in prod.
