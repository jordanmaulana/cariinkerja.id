...
Six scrapers: `indeed.py`, `jobstreet.py`, `linkedin.py`, `dealls.py`, `kalibrr.py`, `kitalulus.py`. Each exposes `crawl(url, max_pages=, limit=, sleep=)` as a generator of posting dicts. The last two are the "clean" sources (see `_docs/scraping-policy.md`): they identify themselves with `jobs.consts.BOT_USER_AGENT` and use `httpx`, no fingerprint impersonation. Kalibrr reads a public JSON API; Kitalulus parses schema.org JSON-LD. A seventh, `karirhub.py`, was deleted on 2026-09-09 — permitted to crawl, but too low-volume/low-quality to be worth the budget, so it went entirely (scraper, `crawl_karirhub` command, hostname branch, URL builder, `Source` member, health target).
...
`jobs/url_builders.py` builds the standard `crawl_urls` set for a Preference (`build_crawl_urls(title, job_types, remote_options)`): **always two URLs** — Kalibrr and Kitalulus, keyword-searched on the Preference title.

`job_types` reaches Kalibrr (repeatable `/t/<tenure-slug>` path segments, forwarded by the scraper as `tenure=`) and Kitalulus (repeatable `types=` query param, values `FULL_TIME`/`PART_TIME`/`CONTRACT`/`INTERNSHIP`), which omits it when all four types are selected since that filters nothing. Kalibrr emits its segments unconditionally — its `tenure` values are OR-ed, so the all-selected case returns the unfiltered set anyway, and `part-time` expands to **two** segments (`part-time` + `freelance`) to mirror `TENURE_TO_JOBTYPE`.

`remote_options` reaches Kalibrr only, as `/work_from_home/y` and only when remote is the *sole* option (Kalibrr can filter *to* WFH, never away from it). It is **ignored** for Kitalulus: its `locationSites` accepts only `REMOTE`/`ON_SITE` — no hybrid — and `REMOTE` matches ~195 postings site-wide, which would starve the crawl. Every remaining remote mismatch is the assessor's job (the score-cap rule in `assessment/services.py`).

Kitalulus URLs also always carry `sortBy=updatedAt`. Its default `isHighlighted` sort is promoted-first and barely moves day to day, so with `CRAWL_ITEM_LIMIT = 10` the daily crawl would keep re-reading postings `assess_job` already skips. Only `isHighlighted`, `updatedAt` and `createdAt` are valid — **an unknown `sortBy` or `types` value renders a valid page with zero results**, which the scraper logs-and-skips rather than raising, so a typo here fails silently. Indeed, JobStreet and LinkedIn were dropped from the generated set on 2026-09-09 (`_docs/scraping-policy.md` §3 exit condition); `build_jobstreet_url`/`build_linkedin_url`, their scrapers, the `crawl_*` commands and the `scraper_for_url` hostname branches all stay, so an admin-pasted URL for those boards still crawls — nothing generates one.
