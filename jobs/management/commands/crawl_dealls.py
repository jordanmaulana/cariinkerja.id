"""Crawl a Dealls listing URL and persist Job rows.

Example:

    uv run manage.py crawl_dealls \\
...
from jobs.scrapers import dealls

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = "Crawl a Dealls listing URL and upsert Job rows by url."

    def add_arguments(self, parser):
        parser.add_argument(
            "url",
            help=(
                "Dealls listing URL (e.g. "
                "https://dealls.com/?location=remote&employment=partTime)"
            ),
        )
        parser.add_argument("--max-pages", type=int, default=1)
        parser.add_argument("--sleep", type=float, default=dealls.DEFAULT_SLEEP)
